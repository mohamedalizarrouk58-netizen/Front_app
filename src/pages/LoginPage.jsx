...
function LoginPage() {
  ...
  const [twoFAData, setTwoFAData] = useState(null) // { username, emailHint }
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (!resolvedRole) {
        throw new Error(t('Login succeeded but role is missing. Please contact support.'))
      }
      ...
      setTimeout(() => navigate(getRouteForRole(resolvedRole), { replace: true }), 3000)
    } catch (requestError) {
  ...
  const handle2faSuccess = (tokens) => {
    ...
    if (!resolvedRole) {
      setError(t('Login succeeded but role is missing. Please contact support.'))
      setIsSubmitting(false)
      return
    }
    ...
    <>
      ...
      {twoFAData && (
        <TwoFAModal
          ...
          onCancel={() => { setTwoFAData(null); setIsSubmitting(false) }}
        />
      )}
      <AnimatePresence>
        {welcomeData && (
      ...
      <div className="hidden lg:flex flex-1 flex-col justify-between relative z-10 px-8 py-4 lg:px-12 lg:py-10">
        ...
        <div className="max-w-2xl mb-8">
           <h1 className="text-5xl font-bold text-slate-900 dark:text-slate-100 mb-6 leading-[1.1] tracking-tight">
              Conseil, DÃƒÂ©veloppement.<br/>Organisation IntÃƒÂ©grÃƒÂ©e.
           </h1>
      ...
      <div className="w-full lg:w-[500px] bg-white dark:bg-slate-900 rounded-3xl p-8 sm:p-12 flex flex-col justify-start relative z-20 shadow-2xl h-full overflow-y-auto">
         <div className="lg:hidden flex justify-center mb-10">
            <img src={logo} alt="Condori Logo" className="h-14 object-contain" />
         </div>

         <div className="mb-6">
            <h2 className="text-[32px] font-bold text-slate-900 dark:text-slate-100 tracking-tight">Bienvenue !</h2>
            <p className="text-slate-500 dark:text-slate-400 mt-2 text-[15px]">Connectez-vous pour accéder à votre espace de gestion.</p>
         </div>
              ...
              <div className="flex items-center justify-between pt-1">
                ...
                <path d="M1 5L4.5 8.5L13 1" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                 ...
                 <button
                   ...
                   className="text-[14px] font-semibold text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-100 transition-colors"
                 >
                   {t('Forgot Password?')}
                 </button>
              </div>
