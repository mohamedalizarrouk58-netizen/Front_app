...
function RoleLayout({ role }) {
  ...
  return (
    <main className="dashboard-grid min-h-screen font-sans selection:bg-[#1ea0d6] selection:text-white">
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-72 overflow-y-auto border-r border-slate-200 dark:border-slate-700/50 bg-[#dce8ee]/95 dark:bg-slate-900/95 p-4 backdrop-blur-xl lg:block shadow-xl">
        <div className="glass-panel h-full flex flex-col gap-6 p-4">
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="p-2">
            <p className="text-xs font-bold uppercase tracking-widest text-[#1ea0d6] mb-1">{role}</p>
            <h2 className="text-xl font-display font-semibold text-slate-900 dark:text-slate-100">Workspace</h2>
          </motion.div>
          ...
          <nav className="space-y-2 flex-1">
            ...
            <div className="pt-4 pb-2">
               <p className="text-xs font-bold uppercase tracking-widest text-slate-400 dark:text-slate-300 dark:text-slate-500 px-3">Modules</p>
            </div>

            {workspace.modules && workspace.modules
              .map((module, idx) => {
              const Icon = module.icon
              const toPath = module.key === 'messages' ? `/${role}/messages` : roleModulePath(role, module.key)
      ...
      <section className="min-h-screen px-4 py-6 lg:pl-[19.5rem] lg:pr-8 relative overflow-hidden ">
          ...
          >
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div className="inline-flex items-center gap-4">
                ...
                <div>
                  <p className="text-xs uppercase tracking-widest text-[#1ea0d6] font-bold">Connected User</p>
